...
import { detectShells } from '../../terminal/shell-detection.js';
 ...
 *
 * Output/exit/title are pushed to whichever window created the session, looked up at emit time
 * (like the updater does) rather than captured once, since the callback outlives any single call.
 */
export function registerTerminalHandlers(workspace: WorkspaceService): void {
  ...
  const service = createTerminalService({
    ...
    onTitle(id, processName) {
      const window = windowsBySession.get(id);
      if (window !== undefined && !window.isDestroyed()) {
        emitToWindow(window, 'terminal:title', { id, processName });
      }
    },
  });

  registerHandler('terminal:listShells', () => ({ shells: detectShells() }));

  registerHandler('terminal:create', ({ id, cols, rows, shellId }, { window }) => {
    ...
    const shell = service.create(id, open.rootPath, cols, rows, shellId);
    return { shell };
  });
