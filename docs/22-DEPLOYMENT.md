...
_or_ quietly degrades fix quality gets halted. The second one is the one nobody thinks to check, and it is
...
That last row deserves emphasis. Our entire thesis is _"we prove the fix works."_ A rising revert-rate means
...
product that keeps the work on the user's machine is _supposed_ to be boring, and ours is.
