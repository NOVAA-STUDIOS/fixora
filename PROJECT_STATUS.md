...
**A7's deferred (non-blocking) findings, explicitly not implemented per instruction:** the feedback
form's client-side max-length check uses untrimmed text while main validates trimmed text, so a
message padded with trailing whitespace can be client-blocked as "too long" even though main would
accept it; the assembled `mailto:` URL has no total-length guard, so a long, unicode-heavy suggestion
can exceed historical Windows mail-client URL limits and be silently truncated with no error
surfaced; suggestion text is stored/exported/emailed verbatim with no secret-scanning or PII warning
(unlike Repair/Proceed's secret-gated paths — a materially different risk profile, since the user
authors every character here); the Gmail Web fallback has no browser-presence pre-check before
`shell.openExternal` (deliberate, mirrors BUG-005's accepted reasoning); history and export are both
